...
contract DCensus {
	...
	emit LogAddStat(msg.sender, _category, _value);
    }
	...
	for (uint i = 0; i < statsData[_category].length; i++) {
		if (statsData[_category][i].owner == msg.sender) {
			emit LogGetStat(msg.sender, _category, statsData[_category][i].value);
			
			return statsData[_category][i].value;
		}
	}

	emit LogGetStat(msg.sender, _category, 'Unknown');
